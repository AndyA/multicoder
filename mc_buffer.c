/* mc_buffer.c */

#include <string.h>

#include "mc_buffer.h"
#include "mc_util.h"

mc_buffer *mc_buffer_new(size_t sz) {
  mc_buffer *b = mc_alloc(sizeof(mc_buffer));
  b->size = sz + 1; // +1 byte because we don't want to ever be full
  b->b = mc_alloc(b->size);
  pthread_mutex_init(&b->mutex, NULL);
  pthread_cond_init(&b->can_read, NULL);
  pthread_cond_init(&b->can_write, NULL);
  return b;
}

void mc_buffer_free(mc_buffer *b) {
  if (b) {
    mc_buffer_reader *br, *next;
    for (br = b->br; br; br = next) {
      next = br->next;
      free(br);
    }
    free(b->b);
    free(b);
  }
}

mc_buffer_reader *mc_buffer_add_reader(mc_buffer *b) {
  mc_buffer_reader *br = mc_alloc(sizeof(mc_buffer_reader));
  br->b = b;
  br->next = b->br;
  b->br = br;
  return br;
}

static size_t _b_get_buf(const mc_buffer *b, mc_buffer_iov *bi,
                         size_t avail, unsigned long pos) {
  bi->iovcnt = 0;

  if (avail) {
    size_t tail = b->size - pos;
    bi->iovcnt++;
    bi->iov[0].iov_base = b->b + pos;
    if (avail <= tail) {
      bi->iov[0].iov_len = avail;
    }
    else {
      bi->iov[0].iov_len = tail;
      bi->iovcnt++;
      bi->iov[1].iov_base = b->b;
      bi->iov[1].iov_len = avail - tail;
    }
  }

  return avail;
}

static size_t _b_space(mc_buffer *b) {
  mc_buffer_reader *br;
  ssize_t space = 0;
  for (br = b->br; br; br = br->next) {
    ssize_t sp = br->pos - b->pos;
    if (sp <= 0) sp += b->size;
    if (space == 0 || sp < space) space = sp;
  }
  if (space > 0) return (size_t) space - 1;
  return 0;
}

static size_t _b_available(mc_buffer_reader *br) {
  mc_buffer *b = br->b;
  ssize_t avail = b->pos - br->pos;
  if (avail < 0) avail += b->size;
  return avail;
}

size_t mc_buffer_space(mc_buffer *b) {
  pthread_mutex_lock(&b->mutex);
  size_t space = _b_space(b);
  pthread_mutex_unlock(&b->mutex);
  return space;
}

size_t mc_buffer_available(mc_buffer_reader *br) {
  mc_buffer *b = br->b;
  pthread_mutex_lock(&b->mutex);
  size_t avail = _b_available(br);
  pthread_mutex_unlock(&b->mutex);
  return avail;
}

size_t mc_buffer_get_input(mc_buffer *b, mc_buffer_iov *bi) {
  return _b_get_buf(b, bi, _b_space(b), b->pos);
}

size_t mc_buffer_get_output(mc_buffer_reader *br, mc_buffer_iov *bi) {
  return _b_get_buf(br->b, bi, _b_available(br), br->pos);
}

void mc_buffer_commit_input(mc_buffer *b, size_t len) {
  if (len) {
    pthread_mutex_lock(&b->mutex);
    b->pos += len;
    if (b->pos >= b->size) b->pos -= b->size;
    pthread_mutex_unlock(&b->mutex);
    pthread_cond_broadcast(&b->can_read);
  }
}

void mc_buffer_commit_output(mc_buffer_reader *br, size_t len) {
  if (len) {
    mc_buffer *b = br->b;
    pthread_mutex_lock(&b->mutex);
    br->pos += len;
    if (br->pos >= b->size) br->pos -= b->size;
    pthread_mutex_unlock(&b->mutex);
    pthread_cond_broadcast(&b->can_write);
  }
}

size_t mc_buffer_wait_input(mc_buffer *b, mc_buffer_iov *bi) {
  size_t sz = 0;
  pthread_mutex_lock(&b->mutex);
  for (;;) {
    sz = mc_buffer_get_input(b, bi);
    if (sz) break;
    pthread_cond_wait(&b->can_write, &b->mutex);
  }
  pthread_mutex_unlock(&b->mutex);
  return sz;
}

size_t mc_buffer_send_input(mc_buffer *b, const void *base, size_t len) {
  const unsigned char *bp = base;
  size_t olen = len;
  while (len > 0) {
    mc_buffer_iov iov;
    size_t done = 0;
    mc_buffer_wait_input(b, &iov);
    for (int i = 0; i < iov.iovcnt; i++) {
      size_t avail = iov.iov[i].iov_len;
      if (avail > len) avail = len;
      memcpy(iov.iov[i].iov_base, bp, avail);
      bp += avail;
      len -= avail;
      done += avail;
    }
    mc_buffer_commit_input(b, done);
  }
  return olen;
}

size_t mc_buffer_wait_output(mc_buffer_reader *br, mc_buffer_iov *bi) {
  mc_buffer *b = br->b;
  size_t sz = 0;
  pthread_mutex_lock(&b->mutex);
  for (;;) {
    sz = mc_buffer_get_output(br, bi);
    if (sz || b->eof) break;
    pthread_cond_wait(&b->can_read, &b->mutex);
  }
  pthread_mutex_unlock(&b->mutex);
  return sz;
}

void mc_buffer_eof(mc_buffer *b) {
  b->eof = 1;
  pthread_cond_broadcast(&b->can_read);
}

/* vim:ts=2:sw=2:sts=2:et:ft=c
 */
