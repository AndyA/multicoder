...
jd_var *mc_model_get(jd_var *v, jd_var *fallback, const char *path, ...) {
  va_list ap;
  va_start(ap, path);
  jd_var *slot = mc_model_vget(v, fallback, path, ap);
  va_end(ap);
  return slot;
}
...
jd_int mc_model_get_int(jd_var *v, jd_int fallback, const char *path, ...) {
  scope {
    va_list ap;
    va_start(ap, path);
    jd_var *slot = mc_model_vget(v, jd_niv(fallback), path, ap);
    va_end(ap);
...
double mc_model_get_real(jd_var *v, double fallback, const char *path, ...) {
  scope {
    va_list ap;
    va_start(ap, path);
    jd_var *slot = mc_model_vget(v, jd_nrv(fallback), path, ap);
    va_end(ap);
...
const char *mc_model_get_str(jd_var *v, const char *fallback, const char *path, ...) {
  scope {
    va_list ap;
    va_start(ap, path);
    jd_var *slot = mc_model_vget(v, jd_nsv(fallback), path, ap);
    va_end(ap);
...
jd_var *mc_model_load_json(jd_var *out, FILE *f) {
  jd_var json = JD_INIT;
  jd_from_json(out, mc_model_load_string(&json, f));
  jd_release(&json);
  return out;
}
...
jd_var *mc_model_load_file(jd_var *out, const char *fn) {
  ...
  jd_var *v = mc_model_load_json(out, fl);
  fclose(fl);
  return v;
}
...
jd_var *mc_model_load(jd_var *stash) {
  ...
  if (!lm || jd_get_int(lm) < st.st_mtime) {
    ...
    mc_model_load_file(jd_get_ks(stash, "model", 1), name);
...
jd_var *mc_model_multi_new(jd_var *stash, jd_var *names) {
  ...
  for (unsigned i = 0; i < nn; i++) {
    mc_model_new(jd_push(slots, 1), jd_get_idx(names, i));
  }
  return stash;
}
...
jd_var *mc_model_multi_load(jd_var *out, jd_var *stash) {
  ...
  for (unsigned i = 0; i < nn; i++)
    jd_append(out, mc_model_load(jd_get_idx(slots, i)));
  return out;
}
